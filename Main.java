...
public class Main extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		JLabel labelLoadedSudokusText = new JLabel( "Enthaelt:" );
		...
		JLabel labelTaskText = new JLabel( "Loese soviele Sudokus wie moeglich - Zeitlimit (in Sekunden):" );
		...
		jTextArea1.setText( "Eigenen SudokuSolver fuer jeden CPU-Kern starten (schneller auf Multicore-Systemen, aber auch schwerer zu debuggen)" );
	...
	private void solveSudokusClicked( java.awt.event.MouseEvent evt ) {//GEN-FIRST:event_solveSudokusClicked
		...
		if( sudokus.size() > 0 ) {
			...
			labelSolvingProgress.setText( "0 von " + sudokus.size() + " " + (sudokus.size() > 1 ? "Sudokus" : "Sudoku") + " geloest" );
				...
				threads.add( new Thread( new Runnable() {
					...
					public void run() {
						...
						if( terminated ) {
							...
							labelSolvingProgress.setText( count.intValue() + " von " + sudokus.size() + " " + (sudokus.size() > 1 ? "Sudokus" : "Sudoku") + " gel̦st" );
				...
				public void run() {
					while( (running.intValue()) > 0 ) {
						...
						try {
							...
							labelSolvingProgress.setText( c + " von " + sudokus.size() + " " + (sudokus.size() > 1 ? "Sudokus" : "Sudoku") + " gel̦st" );
